...
import { ExplainRequestSchema, ExplainResponseSchema } from '@/lib/ai/schemas';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rateCheck = checkRateLimit(`explain-${ip}`, 20, 60_000);
    ...
    const { query, explanationLevel } = parseResult.data;

    // 3. Build prompt and invoke Gemini
    const prompt = buildExplainPrompt(query, explanationLevel);
    ...
    // Distinguish missing key vs generic error
